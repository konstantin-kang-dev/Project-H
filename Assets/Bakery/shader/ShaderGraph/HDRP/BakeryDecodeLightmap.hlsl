...
void BakeryMonoSH_float(float3 normalWorld, float2 lightmapUV, out float3 sh)
{
    ...
    float3 L0 = unity_Lightmap.Sample(samplerunity_Lightmap, lightmapUV);
...
void BakerySpecMonoSH_float(float3 normalWorld, float2 lightmapUV, float3 viewDir, float smoothness, float3 albedo, float metalness,
                                                                                                    out float3 diffuseSH, out float3 specularSH)
{
#ifdef LIGHTMAP_ON
#ifdef DIRLIGHTMAP_COMBINED
...
void BakeryVolume_float(float3 posWorld, float3 normalWorld, out float3 sh)
{
    ...
    tex0 = _Volume0.SampleLevel(sampler_Volume0, lpUV,0);
    tex1 = _Volume1.SampleLevel(sampler_Volume0, lpUV,0);
    tex2 = _Volume2.SampleLevel(sampler_Volume0, lpUV,0);
    L0 = tex0.xyz;
    L1x = tex1.xyz;
    L1y = tex2.xyz;
